...
// Available background images for restaurant cards
const restaurantImages = [
  require("../images/360_F_580781596_uVgdX3avt2KHJQxWr4gd9Xli4VFkcfb5.jpg"),
  require("../images/istockphoto-153775715-612x612.jpg"),
  require("../images/istockphoto-868408746-612x612.jpg"),
];
...
const HomeScreen = () => {
  ...
  const renderRestaurantCard = ({ item, index }) => {
    ...
    const backgroundImage = restaurantImages[index % restaurantImages.length];
    ...
    return (
      <View className="bg-white rounded-2xl mx-4 mb-6 overflow-hidden shadow-lg border border-gray-100">
        ...
        {/* Restaurant Header */}
        <View className="bg-gradient-to-r from-orange-400 to-red-500 px-6 py-8 relative">
        ...
        <View className="p-6 relative">
          ...
          {item.description && (
            <Text
              className="text-white text-sm mb-6 leading-5"
              style={{ fontFamily: "Poppins-Regular" }}
              numberOfLines={2}
            >
              {item.description}
          ...
          {/* Menu Preview */}
          {restaurantMenus[item._id] &&
            restaurantMenus[item._id].length > 0 && (
              <View className="mb-6">
                <Text
                ...
                <View className="space-y-2">
                  {restaurantMenus[item._id].map((menuItem, idx) => (
                    <View
                      key={idx}
                      className="flex-row items-center justify-between bg-black/40 rounded-lg p-3"
                    >
                      <View className="flex-1">
                        <Text
                      ...
                      <Text
                        className="text-white font-bold text-sm ml-3"
                        style={{ fontFamily: "Poppins-Bold" }}
                      >
                        ₹{menuItem.price}
                      </Text>
        ...
        {/* Floating Badge */}
        {index === 0 && (
          <View className="absolute top-4 left-4 bg-yellow-400 rounded-full px-3 py-1 flex-row items-center z-10">
